  ...
  data() {
    return {
      ...
      rules: {
        OtherInOutMoney: [
          {
            required: true, message: '请选择收入方式', trigger: 'blur',
          },
        ],
        Number: [
  ...
  methods: {
    ...
    saveRevenue() {
      ...
      this.$refs.ruleForm.validate((valid) => {
        if (!valid) return;
        this.$api['hand-over/dTOProBusinessAccoutDoneList/otherInMoneyLogPost'](this.ruleForm).then(() => {
          this.$message({
            message: '保存成功',
            type: 'success',
          });
